...
//
//  Created by yingjian on 2017/2/23.
...
extension String {
    ...
    func isTelephone() -> Bool {
        ...
        return isTrue(regex: regex)
//        do {
//            let regular: NSRegularExpression = try NSRegularExpression.init(pattern: regex, options: .caseInsensitive)
//            let matches = regular.matches(in: self, options: NSRegularExpression.MatchingOptions.reportProgress, range: NSMakeRange(0, self.characters.count))
...
//        }
    ...
    // 验证密码
    func regexPassword() -> Bool {
        let regex = "(^[A-Za-z0-9]{6,20}$)"
        return isTrue(regex: regex)
    }
    
    func isTrue(regex: String) -> Bool {
...
//    }
    ...
    // 字符width constrainedToSize 如果一行，用CGSizeZero，否则用CGSizeMake(你想要的宽度, CGFloat(MAXFLOAT)    
